...
def all_baked_goods():
    if request.method == "GET":
        ...
        print("Retrieved all baked goods")
        print(baked_goods)
        ...
        create_baked_good = BakedGood(
            name=request.form.get("name"),
            price=request.form.get("price"),
            bakery_id=request.form.get("bakery_id"),
        )

        db.session.add(create_baked_good)
        db.session.commit()

        baked_good_dict = create_baked_good.to_dict()
...
def delete_baked_good(id):
    ...
    response_body = {"delete_successful": True, "message": "Baked_good Deleted"}
